...
class YouTubeOAuth2Handler(InfoExtractor):
    ...
    def handle_oauth(self, request: yt_dlp.networking.Request):
        ...
        is_youtube = parsed.netloc.endswith('youtube.com')
        is_googlevideo = parsed.netloc.endswith('googlevideo.com')

        if not is_youtube and not is_googlevideo:
            return

        # googlevideo.com CDN: just add Authorization header, no other modifications needed
        if is_googlevideo:
        ...
        # Only apply OAuth2 to the player API — browse/search/next reject the token with 400
        if '/youtubei/v1/' in parsed.path and '/player' not in parsed.path:
            return
    ...
    def authorize(self):
        ...
        while True:
            # Google returns HTTP 400 with {"error":"authorization_pending"} while waiting.
            # Use expected_status to prevent _download_json from raising on 400.
            try:
                token_response = self._download_json(
                    'https://www.youtube.com/o/oauth2/token',
                    video_id='oauth2',
                    ...
                    headers={'Content-Type': 'application/json', '__youtube_oauth__': True},
                    expected_status=[200, 400, 428])
            except Exception:
                time.sleep(code_response.get('interval', 5))
                continue
